...
Matrix SetP((MPC_HP_LEN*SS_Z_LEN), 1);
Matrix x(SS_X_LEN, 1);
Matrix u(SS_U_LEN, 1);
Matrix z(SS_Z_LEN, 1);
...
void loop() {
    ...
    if (timerMPC > SS_DT_MILIS) {
        ...
        for (int32_t _i = 0; _i < (MPC_HP_LEN-1); _i++) {
            SetP = SetP.InsertSubMatrix(SetP, (_i*SS_Z_LEN), 0, ((_i+1)*SS_Z_LEN), 0, SS_Z_LEN, 1);
        }
        SetP = SetP.InsertSubMatrix(SP_NEXT, ((MPC_HP_LEN-1)*SS_Z_LEN), 0, 0, 0, SS_Z_LEN, 1);
        /* -------------------------------- Updating Set Point --------------------------------- */
        ...
        /* =========================== Print to serial (for plotting) ========================== */
            ...
            snprintf(bufferTxSer, sizeof(bufferTxSer)-1, "%.3f %.3f %.3f %.3f %.3f", ((float)u64compuTime)/1000., SetP[0][0], SetP[1][0], z[0][0], z[1][0]);
        #else
