...
pos = 0
theta = np.matrix([[0.99*np.pi], [0.99*np.pi]])
dpos = 0
dtheta = np.matrix('0; 0')
ddpos = 0
ddtheta = np.matrix('0; 0')
u = 0

E_kin_cart = 1 / 2 * m0 * dpos**2
E_kin_p1 = 1 / 2 * m1 * (
    (dpos + l1 * dtheta[0] * np.cos(theta[0]))**2 +
    (l1 * dtheta[0] * np.sin(theta[0]))**2) + 1 / 2 * J1 * dtheta[0]**2
E_kin_p2 = 1 / 2 * m2 * (
    (dpos + L1 * dtheta[0] * np.cos(theta[0]) + l2 * dtheta[1] * np.cos(theta[1]))**2 +
    (L1 * dtheta[0] * np.sin(theta[0]) + l2 * dtheta[1] * np.sin(theta[1]))**
    2) + 1 / 2 * J2 * dtheta[0]**2

E_kin = E_kin_cart + E_kin_p1 + E_kin_p2

E_pot = m1 * g * l1 * np.cos(
theta[0]) + m2 * g * (L1 * np.cos(theta[0]) +
                            l2 * np.cos(theta[1]))

euler_lagrange1 = h1*ddpos+h2*ddtheta[0]*np.cos(theta[0])+h3*ddtheta[1]*np.cos(theta[1]) - (h2*dtheta[0]**2*np.sin(theta[0]) + h3*dtheta[1]**2*np.sin(theta[1]) + u)
euler_lagrange2 = h2*np.cos(theta[0])*ddpos + h4*ddtheta[0] + h5*np.cos(theta[0]-theta[1])*ddtheta[1] - (h7*np.sin(theta[0]) - h5*dtheta[1]**2*np.sin(theta[0]-theta[1]))
euler_lagrange3 = h3*np.cos(theta[1])*ddpos + h5*np.cos(theta[0]-theta[1])*ddtheta[0] + h6*ddtheta[1] - (h5*dtheta[0]**2*np.sin(theta[0]-theta[1]) + h8*np.sin(theta[1]))

euler_lagrange = np.vstack((euler_lagrange1, euler_lagrange2)) #np.matrix([[euler_lagrange1], [euler_lagrange2], [euler_lagrange3]])
euler_lagrange = np.vstack((euler_lagrange,  euler_lagrange3))
